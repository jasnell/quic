...
function connectAfterLookup(session, type, port, ipv6Only, err, ip) {
  ...
  const sc = session[kState].clientSecureContext;

  let flags = 0;
  if (ipv6Only)
    flags |= UV_UDP_IPV6ONLY;
  const handle = _createClientSession(this, type, ip, port || 0, flags, sc);
  if (!handle) {
...
class QuicClientSession extends QuicSession {
  constructor(socket) {
    super(socket);
    const sc = createSecureContext(options);
    this[kState] = {
      clientSecureContext: sc,
      handleReady: false,
      socketReady: false,
    };
  }
